...
class BonfireGame extends CustomBaseGame with KeyboardEvents {
  ...
  Future<void> onLoad() {
    ...
    _initialEnemies?.forEach((npc) => add(npc));
  ...
  Iterable<NPC> visibleEnemies() {
    return _visibleComponents.where((element) => (element is NPC)).cast();
  }

  Iterable<NPC> livingEnemies() {
    return enemies().where((element) => !element.isDead).cast();
  }

  Iterable<NPC> enemies() {
    return components.where((element) => (element is NPC)).cast();
  }
