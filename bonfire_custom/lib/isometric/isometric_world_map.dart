...
import '../Isometric/model/isometric_data_object_oollision.dart';
import '../Isometric/model/isometric_item_tile_set.dart';
import '../Isometric/model/isometric_object_properties.dart';

typedef ObjectBuilder = GameComponent Function(
    IsometricObjectProperties properties);

class IsometricWorldMap {
  static const ORIENTATION_SUPPORTED = 'isometric';
  ...
  Future<IsometricWorldData> build() async {
    try {
      _isometricMap = await _readMap();
      if (_isometricMap?.orientation != ORIENTATION_SUPPORTED) {
        throw Exception(
          'Orientation not supported. please use $ORIENTATION_SUPPORTED orientation',
        );
      }
      _tileWidthOrigin = _isometricMap?.tileWidth?.toDouble() ?? 0.0;
      _tileHeightOrigin = _isometricMap?.tileHeight?.toDouble() ?? 0.0;
      _tileWidth = forceTileSize?.width ?? _tileWidthOrigin;
      _tileHeight = forceTileSize?.height ?? _tileHeightOrigin;
      await _load(_isometricMap!);
    } catch (e) {
      onError?.call(e);
      print('(IsometricWorldMap) Error: $e');
    }

    return Future.value(IsometricWorldData(
      map: MapWorld(_tiles),
      components: _components,
    ));
  }

  Future<void> _load(TiledMap isometricMap) async {
    await Future.forEach<MapLayer>(isometricMap.layers ?? [], (layer) async {
  ...
  Future<IsometricItemTileSet?> _getDataTile(int index) async {
    TileSet? tileSetContain;
    String _pathTileset = '';
    int firsTgId = 0;

    _isometricMap?.tileSets?.forEach(
      (tileSet) {
    ...
    if (tileSetContain != null) {
      ...
      IsometricDataObjectCollision object = _getCollision(
        tileSetContain!,
        (index - firsTgId),
      );
  ...
  IsometricDataObjectCollision _getCollision(
      TileSet tileSetContain, int index) {
    ...
    if ((tileSetItemList.isNotEmpty)) {
      ...
      return IsometricDataObjectCollision(
  ...
  Future<TiledMap> _readMap() async {
    if (fromServer) {
      try {
        TiledMap isometricMap;
        final mapResponse = await http.get(Uri.parse(path));
        isometricMap = TiledMap.fromJson(jsonDecode(mapResponse.body));
        await Future.forEach<TileSetDetail>(isometricMap.tileSets ?? [],
            (tileSet) async {
        ...
        return Future.value(isometricMap);
      } catch (e) {
        print('(IsometricWorldMap) Error: $e');
